...
t2_values = [0,0.03]  # Angular frequency in atomic units
...
for t2 in t2_values:
    ...
    J_t = [calculate_current(solutions, t_index, time_steps, Ht, vertical_edge_indices1) for t_index in range(len(time_steps))]
    ...
    plt.plot(time_steps, J_y_multiplied, label=f'$t2 = {t2}$')

    #plt.plot(time_steps, J_x_multiplied, label=r'$J_{\perp}$', color='red')
plt.xlabel(r'$t$ (a.u.)', fontsize=14)
plt.ylabel(r'$I$ (a.u.)', fontsize=14)
plt.title(f'parallel current components along the edge, flakesize= {m}, frequency = {wavelength_um:.2f}, A0 = {A_0}')
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.legend(fontsize=17)
plt.grid(True)
plt.savefig(f'plots/edge_current_rightmostedge_{num}.pdf')
...
for t2 in t2_values:
    ...
    J_t = [calculate_current(solutions, t_index, time_steps, Ht, vertical_edge_indices2) for t_index in range(len(time_steps))]
    #J_second_pulse = [J_t[index] for index in second_pulse_indices]
...
plt.legend(fontsize=17)
plt.grid(True)
plt.savefig(f'plots/edge_current_leftmostedge_{num}.pdf')
plt.show()
