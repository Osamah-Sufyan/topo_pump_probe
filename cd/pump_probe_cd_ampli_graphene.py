...
# import trial value from via system arguments
...
for i in range(1, num_harmonics + 1):
    ...
    harmonic_integrals_rcp.append(integral_val)
...
# write as circular dichroism
harmonic_integrals = [(harmonic_integrals_rcp[i] - harmonic_integrals_lcp[i])/(harmonic_integrals_rcp[i] + harmonic_integrals_lcp[i]) for i in range(num_harmonics)]
# --- Save to CSV ---
csv_filename = "../data/pp_cd_graphene.csv"
file_exists = os.path.isfile(csv_filename)

with open(csv_filename, mode="a", newline="") as f:
    writer = csv.writer(f)
    if not file_exists:
        header = ["A0"] + [f"Harmonic_{i}" for i in range(1, num_harmonics + 1)]

        writer.writerow(header)
    writer.writerow([0.1] + harmonic_integrals)
