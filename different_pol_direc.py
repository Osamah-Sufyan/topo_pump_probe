...
t2_values = [1,-1]  # Angular frequency in atomic units
...
for factor in t2_values:
        ...
        for t_i in prange(len(t)):
            ...
            for k in range(len(H)):
                for j in range(len(H)):
                    Ht[t_i, k, j] = H[k, j] * np.exp(-1j * (A[k][1] - A[j][1]) * A_x[t_i])* np.exp(-1j * (A[k][2] - A[j][2]) *factor* A_y[t_i])
        return Ht
    ...
    plt.plot(time_steps, J_y_multiplied, label=f'{label_hel}')
...
plt.grid(True)
plt.savefig(f'plots/edge_current_helicity_{num}.pdf')
plt.show()