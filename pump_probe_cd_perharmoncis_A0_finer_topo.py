...
for ind, factor in enumerate(amplitudes):
    ...
    for order in harmonic_orders:
        ...
        integral_arr[ind, order - 1] = abs(total_intensity_right[index] - total_intensity_left[index])/ abs(total_intensity_right[index] + total_intensity_left[index])
...
plt.title('Integral Difference vs Harmonic Order')
plt.xticks(harmonic_orders)
plt.legend()
plt.grid(True)
#plt.savefig('integral_difference_vs_harmonic_order.pdf')
plt.show()