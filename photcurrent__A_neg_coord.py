...
values = np.linspace(1, 1e-3, num=20)
...
# Original time setup
    ...
    @njit
    def calculate_current(solutions, t_index, time_steps, Ht, vertical_edge_indices, vertical_edge_indices1, vertical_edge_indices2):
    ...
    J_t = [calculate_current(solutions, t_index, time_steps, Ht, vertical_edge_indices_all, vertical_edge_indices1, vertical_edge_indices2) for t_index in range(len(time_steps))]
...
plt.plot(amplitudes, current_x, label = r"$\overline{J_x}$")
plt.plot(amplitudes, current_y, label = r"$\overline{J_y}$")
plt.xlabel(r'$A_0$ (a.u.)', fontsize=14)
plt.ylabel(r'$I_p$ (a.u.)', fontsize=14)
plt.title(f'parallel current components along the edge, (right handed helicity) flakesize= {m}, frequency = {wavelength_um:.2f}, t_0 = 0.1')
plt.xticks(fontsize=14)
plt.yticks(fontsize=14)
plt.legend(fontsize=17)
plt.grid(True)
plt.savefig(f'plots/edge_photocurrents_all_neg_coord_A0_{num}.pdf')
plt.show()