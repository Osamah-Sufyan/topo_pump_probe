...
for ind, factor in enumerate(amplitudes):
    ...
    wanted_frequencies = positive_frequencies[:harmonic_10_index]

    


    plt.plot(positive_frequencies[:harmonic_10_index], 1e12 * total_intensity[:harmonic_10_index])
    harmonic_orders = np.arange(1, 13)  # Harmonic orders from 1 to 12
    areas = {}

    # Define frequency window size around each harmonic (e.g., ±0.5 * omega_0)
    delta_omega = 0.5 * omega_0

    for n in harmonic_orders:
        harmonic_freq = n * omega_0
        mask = (positive_frequencies >= harmonic_freq - delta_omega) & (positive_frequencies <= harmonic_freq + delta_omega)
        
        freq_slice = positive_frequencies[mask]
        intensity_slice = total_intensity[mask]
        
        # Compute area under the curve in that window using trapezoidal rule
        area = np.trapz(intensity_slice, freq_slice)
        areas[n] = area

    # Print the results
    for n in harmonic_orders:
        print(f"Area around the {n}th harmonic (ω = {n}*ω₀): {areas[n]:.4e}")
        integral_arr[ind,n-1] = areas[n]
        print(f"Integral for harmonic {n}: {integral_arr[ind,n-1]}")
    ...
    # save as csv file the results
    

    #plt.show()


# save as csv file the results A0 vs the integral for each harmonic
integral_arr = np.array(integral_arr)

# the first column should be the A0 values

integral_arr = np.column_stack((amplitudes, integral_arr))
np.savetxt('integral_results_trivial.csv', integral_arr, delimiter=',', header='A0,' + ','.join([f'Harmonic {i+1}' for i in range(integral_arr.shape[1] - 1)]), comments='')
